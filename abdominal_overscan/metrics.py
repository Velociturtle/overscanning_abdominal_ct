...
def generate_summary() -> None:
    """Generate overscan metrics and summary CSV files."""
    ...
    df["caudal_overscan?"] = (
        pd.to_numeric(df["caudal_overscan_mm"], errors="coerce") > caudal_thresh
    ).map({True: "yes", False: "no"})
    df["cranial_overscan?"] = (
        pd.to_numeric(df["cranial_overscan_mm"], errors="coerce").gt(30)
    ).map({True: "yes", False: "no"})
    df["overscanning?"] = (
        (df["caudal_overscan?"] == "yes") | (df["cranial_overscan?"] == "yes")
    ).map({True: "yes", False: "no"})

    df["calc_caudal_overscan_mm"] = (
        pd.to_numeric(df["caudal_overscan_mm"], errors="coerce") - caudal_thresh
    ).clip(lower=0)
    df["calc_cranial_overscan_mm"] = (
        pd.to_numeric(df["cranial_overscan_mm"], errors="coerce") - 30
    ).clip(lower=0)
    df["calc_total_overscan_mm"] = (
        df["calc_caudal_overscan_mm"] + df["calc_cranial_overscan_mm"]
    )

    scan_length = (
        pd.to_numeric(df["scan_end_z_mm"], errors="coerce")
        - pd.to_numeric(df["scan_start_z_mm"], errors="coerce")
    ).replace(0, pd.NA)
